...
notifyC = function(player, message, type)
    triggerEvent("Notify", player, message)
end
...
function getPesoItem(idItem, quantity)
    if config["Itens"][tonumber(idItem)] then
        return (config["Itens"][tonumber(idItem)][3]*quantity)
    end
    return 0
end

function getItemSpecial(idItem)
    if config["Itens Special"][tonumber(idItem)] then
        return true
    end
    return false
end

function getNameItem(idItem)
--[[     iprint(idItem) ]]
    if config["Itens"][tonumber(idItem)] then
        return config["Itens"][tonumber(idItem)][1]
    end
    return false
end