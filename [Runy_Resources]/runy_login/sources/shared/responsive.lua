...
function createEditBox(x, y, w, h, color, fontType, typeBox)
    if typeBox == 2 then
        if guiGetText(edits[typeBox]) ~= "Senha" then
            if (dxGetTextWidth((select == typeBox and string.gsub(guiGetText(edits[typeBox]), '.', '*')..'|' or string.gsub(guiGetText(edits[typeBox]), '.', '*')), 1.00, fontType) <= w - 10) then
                dxDrawText((select == typeBox and string.gsub(guiGetText(edits[typeBox]), '.', '*')..'|' or string.gsub(guiGetText(edits[typeBox]), '.', '*')), x + 5, y, w - 10, h, color, 1.00, fontType, "left", "center", true, false, false, false, false)
            else
                dxDrawText((select == typeBox and string.gsub(guiGetText(edits[typeBox]), '.', '*')..'|' or string.gsub(guiGetText(edits[typeBox]), '.', '*')), x + 5, y, w - 10, h, color, 1.00, fontType, "right", "center", true, false, false, false, false)
            end
        else
    ...
    elseif typeBox == 3 then
        if guiGetText(edits[typeBox]) ~= "Confirmação de senha" then
            if (dxGetTextWidth((select == typeBox and string.gsub(guiGetText(edits[typeBox]), '.', '*')..'|' or string.gsub(guiGetText(edits[typeBox]), '.', '*')), 1.00, fontType) <= w - 10) then
                dxDrawText((select == typeBox and string.gsub(guiGetText(edits[typeBox]), '.', '*')..'|' or string.gsub(guiGetText(edits[typeBox]), '.', '*')), x + 5, y, w - 10, h, color, 1.00, fontType, "left", "center", true, false, false, false, false)
            else
                dxDrawText((select == typeBox and string.gsub(guiGetText(edits[typeBox]), '.', '*')..'|' or string.gsub(guiGetText(edits[typeBox]), '.', '*')), x + 5, y, w - 10, h, color, 1.00, fontType, "right", "center", true, false, false, false, false)
