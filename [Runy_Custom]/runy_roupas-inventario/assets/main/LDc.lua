...
local cameras = {
    ...
    {616.63, 607.33, 11.09, 8.63, 'cabelo'};
    {644.37, 607.33, 11.09, 8.63, 'camisa'};
    {672.1, 607.33, 11.09, 8.63, 'perna'};
    {699.84, 607.33, 11.09, 8.63, 'tenis'};
}

local windows = {
    {923.79, 252.6, 10.76, 5.98, 'bandana'};
    {992.35, 252, 8.37, 7.18, 'corpo'};
    {1059.13, 252, 8.37, 7.18, 'perna'};
    {1125.9, 252, 8.37, 7.18, 'tenis'};
    {1193.05, 252, 8.37, 7.18, 'cabelo'};
    {1259.46, 252, 8.37, 7.18, 'barba'};
    {924.98, 284.89, 8.37, 7.18, 'mochila'};
    {991.75, 286.08, 9.57, 4.78, 'oculos'};
}
...
function drawClothes()
    ...
    for i, v in ipairs(windows) do 
        ...
        if (isMouseInPosition(905.25 + x, 247.81 + y, 47.84, 14.95) or window == v[5]) then 
            
            dxDrawImage(905.25 + x, 247.81 + y, 47.84, 14.95, 'assets/images/bg_category.png', 0, 0, 0, tocolor(109, 40, 217, alpha))
            dxDrawImage(v[1], v[2], v[3], v[4], 'assets/images/categorys/'..v[5]..'.png', 0, 0, 0, tocolor(255, 255, 255, alpha))
            
        else 
            
            dxDrawImage(905.25 + x, 247.81 + y, 47.84, 14.95, 'assets/images/bg_category.png', 0, 0, 0, tocolor(255, 255, 255, 20))
    ...
    for i, v in ipairs(itens[window]) do
        if config.itensX[v] then
            if (i > page and line < #slots) then 
                dxDrawImage(slots[i][1]-15, slots[i][2], slots[i][3]+28, slots[i][4], config.itensX[v][4], 0, 0, 0, (isMouseInPosition(slots[i][1], slots[i][2], slots[i][3], slots[i][4]) and tocolor(255, 255, 255, alpha) or tocolor(255, 255, 255, alpha * 0.5)))
            end
        end
    end

    dxDrawImage(912, 175, 27.69, 27.69, 'assets/images/male.png', 0, 0, 0, (isMouseInPosition(912, 175, 27.69, 27.69) and tocolor(255, 255, 255, alpha) or tocolor(200, 200, 200, alpha)))
    dxDrawImage(947.69, 175, 27.69, 27.69, 'assets/images/female.png', 0, 0, 0, (isMouseInPosition(947.69, 175, 27.69, 27.69) and tocolor(255, 255, 255, alpha) or tocolor(200, 200, 200, alpha)))
    ...
    if (b == 'left' and s == 'down') then 
        if (isEventHandlerAdded('onClientRender', root, drawClothes)) then 
            ...
            elseif (isMouseInPosition(697.37, 630.75, 86.28, 20.34)) then
                removeClothes()
            elseif (isMouseInPosition(912, 175, 27.69, 27.69)) then
                triggerServerEvent('setPerson', resourceRoot, localPlayer, 1)
            elseif (isMouseInPosition(947.69, 175, 27.69, 27.69)) then
                triggerServerEvent('setPerson', resourceRoot, localPlayer, 10)
            else 
                for i, v in ipairs(cameras) do 
                ...
                local x, y = 0, 0 
                for i, v in ipairs(windows) do 
                    if (isMouseInPosition(905.25 + x, 247.81 + y, 47.84, 14.95)) then 
                        window = v[5]
                        page = 0
                        return 
                    end
                ...
                for i, v in ipairs(itens[window]) do 
                    if (i > page and line < #slots) then 
                        ...
                        if (isMouseInPosition(slots[i][1], slots[i][2], slots[i][3], slots[i][4])) then 
                            triggerServerEvent('onPlayerUseClotheItem', resourceRoot, localPlayer, v)
                        elseif (isMouseInPosition(remove[1][1], remove[1][2], remove[1][3], remove[1][4])) then
                            triggerServerEvent('onPlayerRemoveClotheItem', resourceRoot, localPlayer, window)
                        end
                    end
                end
            end
...
addEvent('onClientDrawClothesInventory', true)
addEventHandler('onClientDrawClothesInventory', root, function(clothes_, type)
    if type == 'update' then
        clothes = clothes_
        itens = {}
...
function removeClothes()
    if (isEventHandlerAdded('onClientRender', root, drawClothes)) then 
        ...
        showCursor(false)
        if isElement(lobby) then
            destroyElement( lobby )
        end
        triggerEvent("returnToLobbyCancelMatch", localPlayer, localPlayer)
    end
end
addEvent('onPlayerRemoveClothesInventory', true)
addEventHandler('onPlayerRemoveClothesInventory', localPlayer, removeClothes)
